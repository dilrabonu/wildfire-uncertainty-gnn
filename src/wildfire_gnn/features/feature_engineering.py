...
def transform_continuous_features(
    ...
    """
    Transform skewed continuous features before GNN training.
    """
    df = df.copy()
...
def add_neighborhood_summary_features(data: Data, add_mean: bool = True, add_std: bool = True) -> Data:
    """
    Adds simple neighborhood statistics from the current node feature matrix.
    Works on a homogeneous graph using mean/std over incoming neighbors.
    """
    data = data.clone()
    x = data.x
...
def build_engineered_feature_table(
    ...
    feature_names_out = [c for c in df.columns if c not in ["target", "split", "row_index", "col_index"]]
