...
def make_random_split(
    ...
    """Create random train/val/test splits using row indices."""
    if not np.isclose(train_size + val_size + test_size, 1.0):
        raise ValueError("Random split sizes must sum to 1.0")
...
def make_spatial_block_split(
    ...
    """Create spatial block split using row/col coordinates."""
    if not np.isclose(train_blocks + val_blocks + test_blocks, 1.0):
        raise ValueError("Spatial block proportions must sum to 1.0")
    ...
    if n_train <= 0 or n_val <= 0 or n_test <= 0:
        raise ValueError("Too few blocks for requested spatial split proportions.")
